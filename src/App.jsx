...
function doExport(transactions,comments,{autoPayments=[],reminders=[],budgets={},customCats=[],widgetConfig=[],profile={},splitGroups=[],splitExpenses=[],settlements=[]}={}) {
...
// ── Transaction Row ───────────────────────────────────────────────────────────
function TxnRow({t,theme,onCatChange,onAddReminder,comment,onCommentSave,allCats,reimbursed,onMarkReimbursed,allTransactions,lang="es",onSplitTx,splitExpenses=[]}) {
  const {c,font:f}=theme;
  const tr=T[lang]||T.en;
  const catLabel=cat=>(CAT_LABELS[lang]||CAT_LABELS.en)[cat]||cat;
  const isMobile=useIsMobile();
...
// ── SplitModal — triggered from TxnRow ───────────────────────────────────────
function SplitModal({theme,lang,tx,splitGroups,splitExpenses,onSave,onClose}){
  const{c,font:f}=theme;const isMobile=useIsMobile();
...
// ── SplitsPage ────────────────────────────────────────────────────────────────
function SplitsPage({theme,lang,splitGroups,setSplitGroups,splitExpenses,setSplitExpenses,settlements,setSettlements,transactions,onMarkReimbursed,showToastFn}){
  const{c,font:f}=theme;const isMobile=useIsMobile();
...
export default function App() {
  ...
  const [splitTxModal,setSplitTxModal]     = useState(null); // transaction to split
  ...
  const handleExport=useCallback(()=>doExport(transactions,comments,{autoPayments,reminders,budgets,customCats,widgetConfig,splitGroups,splitExpenses,settlements,profile:{userName:user?.name||"",lang,themeId:user?.themeId||"nature",avatar:user?.avatar||"",goal:user?.goal||"know"}}),[transactions,comments,autoPayments,reminders,budgets,customCats,widgetConfig,splitGroups,splitExpenses,settlements,user,lang]);
  const isMobile=useIsMobile();
